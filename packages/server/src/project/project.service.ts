...
import { include } from 'src/utils/compare.string';
...
export class ProjectService {
  ...
  async create(admin: Admin, createProjectDto: CreateProjectDto) {
    if (admin.role !== 'admin' && !include(admin.role, createProjectDto.club)) {
  ...
  async update(admin: Admin, id: number, updateProjectDto: UpdateProjectDto) {
    if (admin.role !== 'admin' && !include(admin.role, updateProjectDto.club)) {
      throw new HttpException('Not admin of club', HttpStatus.UNAUTHORIZED);
    }
    if (admin.role !== 'admin') {
  ...
  async remove(admin: Admin, id: number) {
    ...
    if (admin.role !== 'admin' && !include(admin.role, item?.club)) {
      throw new HttpException('Not admin of club', HttpStatus.UNAUTHORIZED);
    }
